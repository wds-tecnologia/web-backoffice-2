...
3. Para cada resposta 200, o front recebe o JSON acima e abre o modal de revisão (um por vez se houver vários).
4. Usuário revisa/edita e confirma; o front preenche a invoice (número, data, produtos com `_imeis`) e, ao salvar a invoice, envia os IMEIs para o endpoint de IMEIs (já existente).

Com isso, backend e front ficam alinhados para o recebedor de PDF (single e em massa).
